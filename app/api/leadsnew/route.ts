...
export async function GET(request: NextRequest) {
    try {
        ...
        const result = await prisma[resource].findMany({
            skip,
            take,
            include: { address: true, zipcode: true, problem: true, problemtype: true }
        });
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { zipcodeId, problemId, problemtypeId, title, description, firstName, lastName, city, email, phone } = data
        const address = { firstName, lastName, city, email, phone, postalCode: zipcodeId }
        const res = await prisma[resource].create({
            data: {
                zipcodeId, problemId, problemtypeId, title, description, address: {
                    create: address
                }
            }
        });
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        const { zipcodeId, problemId, problemtypeId, title, description, status, firstName, lastName, city, email, phone, postalCode } = data
        const address = { firstName, lastName, city, email, phone, postalCode: zipcodeId }
        const res = await prisma[resource].update({
            where: { id },
            data: { zipcodeId, problemId, problemtypeId, title, description, status, address: { update: address } },
