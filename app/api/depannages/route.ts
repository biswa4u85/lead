...
export async function POST(request: NextRequest) {
    try {
        ...
        const { depannageCategoryId, title, description, firstName, lastName, city, email, phone, postalCode, full_address } = data
        const address = { firstName, lastName, city, email, phone, postalCode }
        const res = await prisma[resource].create({
            data: {
                title, description, depannageCategoryId, full_address, assignStatus: "Draft",
                address: {
                    create: address
                }
            }
...
async function conformEmail(address: any) {
    // Email to Customer
    let title = language?.customer_emails?.receipt_title + "demande"
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        const { depannageCategoryId, title, description, status, firstName, lastName, city, email, phone, postalCode, assignStatus, autoAssign } = data
        const address = { firstName, lastName, city, email, phone, postalCode }
        const res = await prisma[resource].update({
            where: { id },
            data: { depannageCategoryId, title, description, status, assignStatus, address: { update: address } },
